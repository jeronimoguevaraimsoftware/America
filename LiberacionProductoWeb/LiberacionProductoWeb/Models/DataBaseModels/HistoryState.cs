using LiberacionProductoWeb.Models.DataBaseModels.Base;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace LiberacionProductoWeb.Models.DataBaseModels
{
    public class HistoryStates : Entity
    {
        public int ProductionOrderId { get; set; }
        public string User { get; set; }
        public DateTime Date { get; set; }
        public string State { get; set; }
        public string Type { get; set; }

        public override IEnumerable<ReportAuditTrail> AuditTrailComparison(Entity objectToCompare, Entity objectToCompareOld = null, string DistribuitionBatch = null)
        {
            return new List<ReportAuditTrail>();
        }
    }

    public class HistoryStateType
    {
        private HistoryStateType(string value) { Value = value; }

        public string Value { get; private set; }

        public static HistoryStateType OrdenProduccion { get { return new HistoryStateType("OP"); } }
        public static HistoryStateType OrdenAcondicionamiento { get { return new HistoryStateType("OA"); } }

    }
}
